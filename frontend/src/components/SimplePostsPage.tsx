...
function SimplePostsPage() {
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      setError((err as Error).message || "Fehler beim Laden der Posts");
  ...
  const handleContact = async (post: ExtendedPost) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/posts/${post.id}/contact`,
        {
          method: "POST",
          headers: {
  ...
  const filteredPosts = posts
    .filter((post) => {
      ...
      const matchesOwner =
        !showMyPostsOnly || post.user_id === simpleApi.getCurrentUserId();
      ...
      >
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <button
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.backgroundColor = showMyPostsOnly
                ? "#3b82f6"
                : "#f3f4f6";
            }}
          >
              ...
              >
                <div
                  style={{
                    ...
                    height: "40px",
                    backgroundColor:
                      post.type === "request" ? "#f59e0b" : "#10b981",
                    borderRadius: "50%",
                    display: "flex",
                    alignItems: "center",
                ...
                <span
                  style={{
                    backgroundColor:
                      post.type === "request" ? "#fef3c7" : "#d1fae5",
